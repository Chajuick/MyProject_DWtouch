...
export default function CartPurchasePage({ checkCartInfo, setCurrentStatus, navigate }) {
  ...
  function loadUserCouponInfo() {
    ...
    // 서버로 POST 요청 보내기
    fetch('http://192.168.0.7:3001/api/coupon-point/user-coupon-loading', {
      method: 'POST',
      headers: {
...
async function delFromCart() {
  if (purchaseInfo && purchaseInfo.length > 0) {
    ...
    try {
      const response = await fetch('http://192.168.0.7:3001/api/cart/infoDel', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
...
async function addToTerm(termEvent) {
  ...
  try {
    const response = await fetch('http://192.168.0.7:3001/api/term/addToTermCondition', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
async function addToOrder() {
  ...
  try {
    const response = await fetch('http://192.168.0.7:3001/api/order/addToOrder', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
async function delFromUserCoupon() {
  if (purchaseInfo && purchaseInfo.length > 0 && hasUsedCoupon(purchaseInfo)) {
    ...
    try {
      const response = await fetch('http://192.168.0.7:3001/api/coupon-point/user-coupon-delete', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
...
async function subPoint() {
  if (usingPoints > 0) {
    ...
    try {
      const response = await fetch('http://192.168.0.7:3001/api/coupon-point/sub-point', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
