    ...
    if (orderInfo && orderInfo.length > 0) {
      ...
      const filteredOrderInfoArray = orderInfo
        .map(order => {
          ...
          // createdAt에 18시간을 더함
          const newDate = new Date(createdAtDate.getTime() + 9 * 60 * 60 * 1000);
          // 새로운 createdAt을 가진 객체를 반환
          return { ...order, createdAt: newDate.toISOString() };
        })
        .filter(order => {
    ...
    <>
      ...
      <GradeProfileOverView>
        ...
        <GradeProfileDetailInfo>
          ...
          {console.log(NeedAmount(0, userGrade, filteredOrderInfo.length))}
        </GradeProfileDetailInfo>
      </GradeProfileOverView>
      <GradeInfoOverView>
